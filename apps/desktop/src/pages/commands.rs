...
/// 按 config [transfer].udp_enabled 注入传输子进程环境：
/// false → FAN_NO_UDP=1（引擎 transfer.rs 的开关：跳过 UDP 打洞相位，强制 relay），
/// true → 不设（默认行为）。
fn apply_udp_env(cmd: &mut tokio::process::Command, udp_enabled: bool) {
    if !udp_enabled {
        cmd.env("FAN_NO_UDP", "1");
    }
}
...
pub(crate) async fn share_dataset(app: tauri::AppHandle, path: String) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        apply_udp_env(&mut cmd, udp_enabled);
...
mod tests {
    ...
    /// 开启（默认）→ 不注入。
    #[test]
    fn apply_udp_env_injects_fan_no_udp_when_disabled() {
        let mut cmd = tokio::process::Command::new("echo");
        apply_udp_env(&mut cmd, false);
        let envs: Vec<(std::ffi::OsString, Option<std::ffi::OsString>)> = cmd.get_envs().collect();
        assert!(
            envs.iter().any(|(k, v)| k == "FAN_NO_UDP" && v.as_deref() == Some(std::ffi::OsStr::new("1"))),
            "udp_enabled=false 必须注入 FAN_NO_UDP=1: {envs:?}"
        );

        let mut cmd = tokio::process::Command::new("echo");
        apply_udp_env(&mut cmd, true);
        assert!(
            !cmd.get_envs().any(|(k, _)| k == "FAN_NO_UDP"),
            "udp_enabled=true 不得注入 FAN_NO_UDP"
        );
    }
}
